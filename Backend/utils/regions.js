const REGIONS = {
    ...
    "Islamabad Capital Territory & Margalla Region": {
        id: "Islamabad Capital Territory & Margalla Region",
...
// Data copied from Frontend/src/data/regionCoordinates.js
const CITY_COORDINATES = {
    ...
    // KP - Kaghan-Mansehra-Siran
    'Mansehra': [34.3333, 73.2000, 12],
    'Manshera': [34.3333, 73.2000, 12], // Alias for common misspelling
    ...
    'Karachi (Clifton Urban Forest zone)': [24.860966, 66.990501, 14],
    ...
    'Neelum Valley towns': [34.5, 73.9, 10], // Valley general
    ...
    // Sub-Regions (Neighborhoods/Sectors)
    // Peshawar
...
const getAllRegions = () => Object.values(REGIONS);
...
const getRegionById = (id) => {
    if (!id) return null;
    const normalizedInput = id.toString().trim().toLowerCase();

    // 1. Check Standard Provinces/Zones (Static BBOX)
    for (const key in REGIONS) {
        if (key.toLowerCase() === normalizedInput) return REGIONS[key];
    }

    // 2. Check City Coordinates (Dynamic BBOX)
    for (const key in CITY_COORDINATES) {
        if (key.toLowerCase() === normalizedInput) {
            const [centerLat, centerLng, zoom] = CITY_COORDINATES[key];

            // Special Case for "Pakistan" or "All Regions"
            if (key === 'Pakistan' || key === 'All Regions') {
                const bbox = [23.5, 60.5, 37.5, 78.0]; // Full Pakistan
                return {
                    id: key,
                    name: key,
                    bbox: bbox,
            ...
            // Generate approx 10km x 10km box (0.1 deg) centered on point
            const offset = 0.05;
            const bbox = [
                centerLat - offset,
                centerLng - offset,
    ...
    // 3. Last Resort: Partial/Typo match for cities (e.g. "Manshera" vs "Mansehra" if not handled)
    // We check if input is a subset or if names match after basic cleanup
    for (const key in CITY_COORDINATES) {
        const cleanKey = key.toLowerCase().replace(/[^a-z]/g, '');
        const cleanInput = normalizedInput.replace(/[^a-z]/g, '');
        if (cleanKey === cleanInput) {
            return getRegionById(key); // Recursive call with exact key
        }
    }
...
const validateRegion = (id) => !!getRegionById(id);
